...
def meta_test(model, head, adaptation_steps, loss_name, device, exp_group_dir, exp, sparsity_mode):
...
def main(n_shots=5,             # Number of shots of each task.
    ...
    exp_group_name = 'anil_%s_%s_%s_%s' % (network_name, loss_name, trg_dataset, trg_task)
...
#     normalization = 'z-score'
    ...
    for fold in folds:
        ...
        # Testing the models.
        meta_test(model, head, adaptation_steps, loss_name, device, exp_group_dir, exp_pnts, 'points')
        meta_test(model, head, adaptation_steps, loss_name, device, exp_group_dir, exp_grid, 'grid')
        meta_test(model, head, adaptation_steps, loss_name, device, exp_group_dir, exp_scrb, 'scribbles')
        meta_test(model, head, adaptation_steps, loss_name, device, exp_group_dir, exp_cntr, 'contours')
        meta_test(model, head, adaptation_steps, loss_name, device, exp_group_dir, exp_skel, 'skeleton')
        meta_test(model, head, adaptation_steps, loss_name, device, exp_group_dir, exp_poly, 'polygons')
        meta_test(model, head, adaptation_steps, loss_name, device, exp_group_dir, exp_dens, 'dense')
