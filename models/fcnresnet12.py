...
class FCNResNet12(nn.Module):

    def __init__(self, input_channels, output_channels=2, wider=True, final=True):
        
        super(FCNResNet12, self).__init__()
        ...
        self.backbone = ResNet12Backbone(channels=input_channels, wider=wider)

        if self.wider:
            self.final_input_channels = 160
        else:
            self.final_input_channels = 128
        if self.has_final:
